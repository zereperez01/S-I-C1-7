...
public class Wilson {
	...
	public Celda[][] algoritmoWilson(Celda [][] M) {
		...
		byte eleccion=0;
		Stack<Celda> recorrido = new Stack<Celda>(); 
			while(!laberintoCreado(M)) {
				...
				if(!M[fila_posicion][columna_posicion].devolverVisitado()) {
					...
					recorrido.clear();
					recorrido.push(M[fila_posicion][columna_posicion]);
	...
	public boolean comprobarExistenciaBucle(Celda [][] M, Stack<Celda> recorrido, byte fila_posicion, byte columna_posicion) {
	...
	public void deshacerBucle(Celda [][] M, Stack<Celda>recorrido, byte fila_posicion, byte columna_posicion) {
		boolean bucleDeshecho = false;
			while(!bucleDeshecho) {
				if(!(recorrido.peek().equals(M[fila_posicion][columna_posicion]))) {
					recorrido.peek().reponerParedes();
					recorrido.pop();
				}else {
					recorrido.peek().reponerParedes();
					recorrido.peek().cambiarVisitado();
					if(fila_posicion-1>=0 && !M[fila_posicion][columna_posicion].devolverNorte() && M[fila_posicion-1][columna_posicion].devolverSur())
						M[fila_posicion][columna_posicion].cambiarNorte();
					if(columna_posicion+1<M[0].length && !M[fila_posicion][columna_posicion].devolverEste() && M[fila_posicion][columna_posicion+1].devolverOeste())
						M[fila_posicion][columna_posicion].cambiarEste();
					if(fila_posicion+1<M.length && !M[fila_posicion][columna_posicion].devolverSur() && M[fila_posicion+1][columna_posicion].devolverNorte())
						M[fila_posicion][columna_posicion].cambiarSur();
					if(columna_posicion-1>=0 && !M[fila_posicion][columna_posicion].devolverOeste() && M[fila_posicion][columna_posicion-1].devolverEste())
						M[fila_posicion][columna_posicion].cambiarOeste();
					bucleDeshecho=true;
				}
			}
